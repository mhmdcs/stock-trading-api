...
async def create_rule(data: RuleCreate, db: AsyncSession = Depends(get_db)):
    return await process_create_rule(db, data.name, data.threshold_price, data.symbol)

@router.get("/", response_model=List[RuleResponse])
async def get_rules(db: AsyncSession = Depends(get_db)):
    return await process_get_all_rules(db)

@router.patch("/{id}", response_model=RuleResponse)
async def update_rule(id: int, data: RuleUpdate, db: AsyncSession = Depends(get_db)):
    return await process_update_rule(db, id, data.name, data.threshold_price, data.symbol)

@router.delete("/{id}", response_model=RuleResponse)
async def delete_rule(id: int, db: AsyncSession = Depends(get_db)):
    await process_delete_rule(db, id)
